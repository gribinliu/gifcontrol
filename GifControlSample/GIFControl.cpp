...
void CGIFControl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if (m_pImage->GetWidth() < rcClient.Width() && m_pImage->GetHeight() < rcClient.Height())
    {
        int x = (rcClient.Width() - m_pImage->GetWidth()) / 2;
        int y = (rcClient.Height() - m_pImage->GetHeight()) / 2;
        g.DrawImage(m_pImage, x, y, m_pImage->GetWidth(), m_pImage->GetHeight());
    }
    else
    {
